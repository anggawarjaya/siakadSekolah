<?php
	...
	class Laporan_nilai extends CI_Controller
	{
		...
		function index()
		{
			$tahun = get_tahun_akademik('id_tahun_akademik');
			$walikelas 		= $this->db->get_where('tbl_walikelas', array('id_guru' => 3))->row_array();
			$seksi = $walikelas['kd_kelas'];
			$kelas 			= "SELECT tk.nama_kelas, tju.nama_jurusan, tm.nama_mapel, ttk.nama_tingkatan 
							  ...
							  WHERE tj.kd_jurusan = tju.kd_jurusan AND tj.kd_kelas = tk.kd_kelas AND tj.kd_mapel = tm.kd_mapel AND tj.kd_tingkatan = ttk.kd_tingkatan AND tj.kd_kelas= '7-A2'";
			$siswa 			= "SELECT ts.nim, ts.nama
							  FROM tbl_riwayat_kelas AS trk, tbl_siswa AS ts 
							  WHERE trk.nim = ts.nim AND trk.kd_kelas = '7-A2' 
							  AND trk.id_tahun_akademik = '1'";
